...
describe RamenStoresController, type: :request do
    ...
    context 'ログイン済みの場合' do
      ...
      context '店舗が存在する場合' do
        ...
        example '店舗の情報が表示されていること' do
          ...
          expect(response.body).to include '0123-45-67'
  ...
  describe 'GET #edit' do
    context '未ログインユーザーの場合' do
      example 'リクエスト失敗' do
        @ramen_store = create(:ramen_store)
        get edit_ramen_store_url @ramen_store
    ...
    context 'ログイン済みの場合' do
      login
      before(:each) do
        @ramen_store = create(:ramen_store, user: user)
        get edit_ramen_store_url @ramen_store
      end
      context '店舗が存在する場合' do
        example 'リクエストが成功すること' do
  ...
  describe 'POST #create' do
    ...
    context 'パラメータが妥当な場合' do
      ...
      example '店舗が登録されること' do
        expect do
          post ramen_stores_url, params: { ramen_store: attributes_for(:ramen_store) }
        end.to change(RamenStore, :count).by(1)
      end

      example 'リダイレクトすること' do
        post ramen_stores_url, params: { ramen_store: attributes_for(:ramen_store) }
    ...
    context 'パラメータが不正な場合' do
      ...
      example 'エラーが表示されること' do
        post ramen_stores_url, params: { ramen_store: attributes_for(:ramen_store, :invalid) }
        expect(response.body).to include '店舗の登録に失敗しました'
      end
    end
  end
  ...
  describe 'PATCH #update' do
    ...
    context 'パラメータが妥当な場合' do
      example 'リクエストが成功すること' do
        patch ramen_store_url @ramen_store, params: { ramen_store: attributes_for(:ramen_store) }
        expect(response.status).to eq(302)
      end

      example '店舗が更新されること' do
        expect do
          patch ramen_store_url @ramen_store, params: { ramen_store: attributes_for(:ramen_store) }
        end.to change { RamenStore.find(@ramen_store.id).name }.from('店舗13').to('店舗14')
      end
    ...
    context 'パラメータが不正な場合' do
      ...
      example 'エラーが表示されること' do
        post ramen_stores_url, params: { ramen_store: attributes_for(:ramen_store, name: '') }
        expect(response.body).to include '店舗の登録に失敗しました'
      end
    end
  end
  ...
  describe 'DELETE #destroy' do
    ...
    example 'リクエストが成功すること' do
      delete ramen_store_url @ramen_store
      expect(response.status).to eq 302
    end
